...
class ClienteModelTest(TestCase):
    ...
    def test_representacao_objeto(self):
        """Esse método testa a representação em string do modelo Cliente. 
        Verificando se o método __str__ retorna a string formatada corretamente."""
        cliente = Cliente.objects.get(id=1)
        self.assertEqual(cliente.__str__(), "[Cliente Teste - 00000000000 - teste@gmail.com]")
    ...
    def test_tamanho_telefone(self):
    ...
    def test_telefone_unico(self):
        """Esse método verifica se o campo 'telefone' do modelo está 
        corretamente configurado para receber valores únicos."""
        cliente = Cliente.objects.get(id=1)
        status_telefone_unico = cliente._meta.get_field('telefone').unique
        self.assertTrue(status_telefone_unico)

    def test_email_unico(self):
        """Esse método verifica se o campo 'email' do modelo está corretamente 
        configurado para receber valores únicos."""
        cliente = Cliente.objects.get(id=1)
    ...
    def test_data_hora_cadastrado_automaticamente(self):
        """Testa se o campo 'cadastrado_em' está registrando a data e a hora de 
        forma automática."""
        cliente = Cliente.objects.get(id=1)
...
class ClienteViewTest(TestCase):
    """Caso de teste para as views do modelo Cliente."""
    ...
    def test_acesso_autenticado_clientes(self):
        """Testa se o status HTTP é 200; que concede acesso a área de 
        gerencimento de clientes para clientes autenticados."""
        resposta = self.client.get(reverse("clientes:clientes"))
        self.assertEqual(resposta.status_code, 200)
        
    def test_acesso_desautenticado_clientes(self):
        """Testa se o status HTTP é 403; que nega o acesso a área de
        gerenciamento de clientes a clientes não autenticados."""
        self.client.logout()
        resposta = self.client.get(reverse("clientes:clientes"))
        self.assertEqual(resposta.status_code, 403)
    
    def test_acesso_desautenticado_cadastrar_cliente(self):
        """Testa se o status HTTP é 403; que nega o acesso a área de
        cadastro de cliente a clientes não autenticados."""
        self.client.logout()
        resposta = self.client.get(reverse("clientes:cadastrar_cliente"))
        self.assertEqual(resposta.status_code, 403)

    def test_acesso_desautenticado_atualizar_cliente(self):
        """Testa se o status HTTP é 403; que nega o acesso a área de
        atualização de cliente a clientes não autenticados."""
        self.client.logout()
        resposta = self.client.get(reverse("clientes:atualizar_cliente", args=[1]))
        self.assertEqual(resposta.status_code, 403)
    
    def test_acesso_desautenticado_detalhes_cliente(self):
        """Testa se o status HTTP é 403; que nega o acesso a área de
        detalhamento de cliente a clientes não autenticados."""
        self.client.logout()
        resposta = self.client.get(reverse("clientes:detalhes_cliente", args=[1]))
        self.assertEqual(resposta.status_code, 403)
    
    def test_acesso_desautenticado_deletar_cliente(self):
        """Testa se o status HTTP é 403; que nega o acesso a área de
        exclusão de cliente a clientes não autenticados."""
        self.client.logout()
